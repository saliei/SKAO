...
def gridding_single(dataset, uvwt, vist, freq, grid, comm):
    ...
    chunksize_bl = len(dataset.baseline_id) // size
    start = rank * chunksize_bl
    if rank < size - 1:
        end = start + chunksize_bl
    else:
        end = len(dataset.baseline_id) - 1

    for (uvw, vis) in zip(uvwt[start:end], vist[start:end]):
...
def gridding_all(dataset, comm):
    ...
    for t in range(10):
        ...
        grid = gridding_single(dataset, uvwt, vist, freq, grid, comm)
    return grid
