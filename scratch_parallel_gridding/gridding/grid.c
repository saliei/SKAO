...
void gridding_omp(double complex *grid ,double *uvw_data, double complex *visibility_data, double *frequency_data) {
    int i, j, k;

#pragma omp parallel for collapse(2) schedule(static) shared(grid, uvw_data, visibility_data, frequency_data)
...
void gridding_mpi_omp(double complex *grid, double *uvw_data, double complex *visibility_data, double *frequency_data) {
    int i, j, k;
    int rank, size;
...
void gridding_simd(double complex *grid, double *uvw_data, double complex *visibility_data, double *frequency_data) {
    int i, j, k;
    ...
    for (i = 0; i < TIMESTEPS; i++) {
        for (j = 0; j < BASELINES; j++) {
            ...
            for (k = 0; k < FREQUENCS; k += 4) {
                ...
                __m256d vis_real = _mm256_loadu_pd((double*)&visibility_data[(i * BASELINES * FREQUENCS) + (j * FREQUENCS) + k]);
                __m256d vis_imag = _mm256_loadu_pd((double*)&visibility_data[(i * BASELINES * FREQUENCS) + (j * FREQUENCS) + k] + 4);

                __m256d freq = _mm256_loadu_pd(&frequency_data[k]);

                double u = uvw_data[(i * BASELINES * 3) + (j * 3) + 0];
                double v = uvw_data[(i * BASELINES * 3) + (j * 3) + 1];

                // compute the indices
                __m256d iu = _mm256_mul_pd(_mm256_set1_pd(u * THETA_OVER_C), freq);
                __m256d iv = _mm256_mul_pd(_mm256_set1_pd(v * THETA_OVER_C), freq);
...
void gridding_simd_mpi(double complex *grid, double *uvw_data, double complex *visibility_data, double *frequency_data) {
    int i, j, k;
    int rank, size;
