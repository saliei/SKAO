...
def calculate_indices_noround(uvw_0, uvw_1, f):
    # f = np.array(f.compute())
    # uvw_0 == np.array(uvw_0.compute())
    # uvw_1 == np.array(uvw_1.compute())
    # f = f.values
    uvw_0 = uvw_0.values
    uvw_1 = uvw_1.values
    # print(type(f))
    # print(type(uvw_0))
    # print(type(uvw_1))
    # iu = (theta * uvw_0 * f / c).round().astype(int, copy=False)
    # iv = (theta * uvw_1 * f / c).round().astype(int,  copy=False)
    # iu = np.rint(theta * uvw_0 * f / c, dtype=np.int32)
    # iv = np.rint(theta * uvw_1 * f / c, dtype=np.int32)
    ...
    for (uvws, visss) in zip(UVW, VIS):
        for (uvw, viss) in zip(uvws, visss):
            for (freq, vis) in zip(FRQ, viss):
                ...
                print("iu: ", iu)
                print("grid[iug, ivg]: ", grid[iu+image_size//2, iv+image_size//2])
